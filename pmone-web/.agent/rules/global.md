...
- **module-dev**: 通用模块开发流程（API ➡️ 类型 ➡️ Composables ➡️ 页面 ➡️ 组件）
- **module-refactor**: 模块重构/迁移流程（目录扁平化、API 迁移、组件提取）

## 🎨 样式分离规则

- Vue SFC 中 `<style>` 块超过 50 行时，必须将样式提取为同目录下的独立 `.scss` 文件，通过 `<style lang="scss" scoped src="./XxxComponent.scss" />` 引入
- 50 行以内的样式可以直接写在 `<style lang="scss" scoped>` 块中
...
4. 开发前必须查阅 `project-infrastructure` 规范，优先复用已有资产
